...
## file path
pretrained_model = './imagenet_vgg16_pretrained.pth'
compressed_model = './imagenet_vgg16_compression.pth'
...
for block_idx in recasting_block_indices:
    ...
    config[2] = round(config[2] * compression_ratio[block_idx])    # apply compression ratio
    ...
    if len(config) == 5:                         
        ...
        config[4] = round(mid_feature * compression_ratio[block_idx])
    else :
        is_bottleneck = False
    ...
    config[1] = round(config[1] * compression_ratio[block_idx])    # apply compression ratio
    
    # Handling corner case: bottleneck block recasting
    if is_bottleneck == True:                         
        # Change next input dim to output dim of target block
        config[1] = round(mid_feature * compression_ratio[block_idx])
